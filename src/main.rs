...
fn main() {
    ...
    for line in file_contents.split(split_delim) {
        for raw in line.trim().split(",") {
            values.push(Point{label: raw.to_string(), count: 0});
        }
    }
    ...
    let mut team_color: [f32; 3] = [
        0.0,
        0.0,
        1.0
    ];
    let mut line_color: [f32; 3] = [
        1.0,
        1.0,
        1.0
    ];
    let mut font_color: [f32; 3] = [
        1.0,
        1.0,
        1.0
    ];
    ...
    while !rl.window_should_close() {
...

        {
            ...
            ui.window("Config")
              .build(|| {
                    ...
                    ui.input_text("Output", &mut output_path_buffer)
                        .build();
                    ...
                    if ui.button("Generate") {
                        ...
                        img.draw_line(30, 600, 50 + values.len() as i32 * 100, 600, norm_to_reg(line_color));

                        for (i, point) in values.iter_mut().enumerate() {
                            img.draw_text(&point.label, 40 + i as i32 * 100, 650, SIZE, norm_to_reg(font_color));
...

        {
            ...
            d.draw_line(30, 600, 50 + (values.len() * 100) as i32, 600, norm_to_reg(line_color));

            for (i, point) in values.iter().enumerate() {
                d.draw_text(&point.label, 40 + i as i32 * 100, 650, SIZE, norm_to_reg(font_color));
